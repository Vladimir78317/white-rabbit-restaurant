/*
================================================================================
 WHITE RABBIT v4.0 — ЕДИНЫЙ СКРИПТ ЧИСТОЙ УСТАНОВКИ
================================================================================
Назначение: развёртывание базы данных WhiteRabbitRestaurant на новом ПК для
приложения White Rabbit (.NET 10, версия интерфейса v3.6).

Требования:
  • Microsoft SQL Server 2019 или 2022;
  • SQL Server Management Studio (SSMS);
  • запуск от учётной записи, имеющей право CREATE/DROP DATABASE.

ВНИМАНИЕ: скрипт удаляет существующую базу WhiteRabbitRestaurant вместе
со всеми её данными и создаёт новую базу. На рабочей базе с важными данными
этот файл запускать нельзя.

После завершения доступны тестовые учётные записи:
  admin    / admin123
  waiter1  / waiter123
  waiter2  / waiter123
  kitchen1 / kitchen123
...
Скрипт собран как чистая установка: в нём включены итоговые версии структуры,
процедур заказов, стоп-листа, бронирований, схем столиков, смен и фильтров.
Для исключения ошибок предыдущих версий здесь не используются проверки с
динамическим THROW/CONCAT и не передаются выражения CONVERT(...) напрямую в EXEC.
================================================================================
*/
GO
...
/* v3.3: таблица стоп-листа. Нужна для меню, кухни и администрирования. */
...
(
    ...
    ClientId            INT NULL,
    TableId             INT NOT NULL,
    WaiterShiftId       INT NULL,
    OrderStatusId       INT NOT NULL,
    ChannelCode         VARCHAR(20) NOT NULL CONSTRAINT DF_CustomerOrder_ChannelCode DEFAULT ('WAITER'),
    GuestCount          TINYINT NOT NULL CONSTRAINT DF_CustomerOrder_GuestCount DEFAULT (1),
    CreatedAt           DATETIME2(0) NOT NULL CONSTRAINT DF_CustomerOrder_CreatedAt DEFAULT (SYSDATETIME()),
    FinalizedAt         DATETIME2(0) NULL,
    Comment             NVARCHAR(500) NULL,

    CONSTRAINT CK_CustomerOrder_Source CHECK
    (
        (ChannelCode = 'WAITER' AND WaiterShiftId IS NOT NULL AND ClientId IS NULL)
        OR
        (ChannelCode = 'CLIENT_APP' AND ClientId IS NOT NULL)
    ),
    CONSTRAINT CK_CustomerOrder_GuestCount CHECK (GuestCount BETWEEN 1 AND 4),
    ...
    CONSTRAINT FK_CustomerOrder_Client
        FOREIGN KEY (ClientId) REFERENCES dbo.Client(ClientId),
...
/* Примеры ручных тестов исключены из установочного файла. */
GO
...
PRINT N'White Rabbit v4.0: итоговые процедуры заказов и сотрудников созданы.';
GO


/* v3.3 compatibility alias for an earlier GUI build. */
CREATE OR ALTER PROCEDURE dbo.sp_GetAdminShiftsFiltered
    @ShiftDate DATE = NULL,
    @WaiterUserId INT = NULL,
    @StatusCode VARCHAR(20) = NULL,
    @ShiftType VARCHAR(20) = NULL
...
END;
GO
PRINT N'White Rabbit v4.0: процедуры совместимости созданы.';
GO


/* ================== ФИНАЛЬНАЯ ПРОВЕРКА ЧИСТОЙ УСТАНОВКИ ================== */
USE WhiteRabbitRestaurant;
GO
SET NOCOUNT ON;
GO
...
VALUES
    ...
    (N'sp_CreateUser', N'P'),
    (N'sp_AdminCreateEmployee', N'P');

SELECT
    N'dbo.' + ObjectName AS [Объект],
    CASE WHEN OBJECT_ID(N'dbo.' + ObjectName, ObjectType) IS NULL
         THEN N'ОТСУТСТВУЕТ'
         ELSE N'OK'
    END AS [Статус]
...
(
    ...
    WHERE OBJECT_ID(N'dbo.' + r.ObjectName, r.ObjectType) IS NULL
)
    THROW 51990, N'Установка завершилась с ошибкой: не созданы все обязательные объекты.', 1;
...
IF COL_LENGTH(N'dbo.CustomerOrder', N'ClientId') IS NULL
   ...
   OR COL_LENGTH(N'dbo.CustomerOrder', N'GuestCount') IS NULL
    THROW 51991, N'Установка завершилась с ошибкой: в CustomerOrder отсутствуют обязательные поля.', 1;

SELECT
    CAST(SERVERPROPERTY(N'ServerName') AS NVARCHAR(256)) AS [SQL Server],
    DB_NAME() AS [База данных],
    N'White Rabbit v4.0: установка базы завершена успешно.' AS [Результат];
GO

PRINT N'White Rabbit v4.0: база данных WhiteRabbitRestaurant полностью создана и проверена.';
GO

/* ================== ДОПОЛНЕНИЕ v4.1: БРОНИ И СОСТАВ ЗАКАЗА ==================
   В чистую установку добавлены итоговые процедуры из SQL_Update_v3_8.
*/
GO
/*
    White Rabbit v3.8
    Разрешение оформления заказа на забронированный столик:
    - официант может создать заказ для назначенного ему столика, даже если на текущее время есть бронь;
    - клиент может создать заказ для свободного столика либо для столика, который забронировал сам;
    - бронь другого клиента по-прежнему блокирует клиентский заказ.

    Скрипт не удаляет данные. Выполняется в SSMS на базе WhiteRabbitRestaurant.
*/
...
IF OBJECT_ID(N'dbo.CustomerOrder', N'U') IS NULL
    THROW 51801, N'Таблица dbo.CustomerOrder не найдена. Сначала установите основную базу WhiteRabbitRestaurant.', 1;

IF OBJECT_ID(N'dbo.Reservation', N'U') IS NULL
   OR OBJECT_ID(N'dbo.ReservationTable', N'U') IS NULL
   OR OBJECT_ID(N'dbo.Client', N'U') IS NULL
    THROW 51802, N'Не найдены таблицы бронирований. Сначала установите основную базу WhiteRabbitRestaurant.', 1;
GO

/*
  Список столиков для клиента.
  В него входят свободные столики и столик с активной бронью самого клиента.
  Столик с активной бронью другого клиента не возвращается.
*/
CREATE OR ALTER PROCEDURE dbo.sp_GetAvailableTables
    @UserId INT = NULL
AS
...
/*
  Бронь не запрещает официанту открыть заказ: он работает с назначенным столиком
  и всё равно не сможет создать второй незавершённый заказ на нём.
*/
CREATE OR ALTER PROCEDURE dbo.sp_CreateOrderForWaiter
    @WaiterUserId INT,
    @TableNumber INT,
...
BEGIN
    ...
    IF @ShiftId IS NULL
        THROW 51327, N'Сначала откройте назначенную или самостоятельную смену. Без открытой смены заказ создать нельзя.', 1;
...
/*
  Клиент может оформить заказ на столик, который забронировал сам.
  Если на это время действует бронь другого клиента, создание заказа блокируется.
*/
CREATE OR ALTER PROCEDURE dbo.sp_CreateClientAppOrder
    @UserId INT,
    @TableNumber INT,
...
BEGIN
    ...
    (
        ...
        THROW 51335, N'Этот столик забронирован другим клиентом на текущее время.', 1;
...
PRINT N'White Rabbit v3.8: заказы на забронированные столики настроены.';
GO

PRINT N'White Rabbit v4.1: база данных создана с поддержкой заказов на забронированных столиках.';
GO
